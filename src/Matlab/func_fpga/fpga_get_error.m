function [error_map_uart, error_val_uart] = fpga_get_error(serial_obj, bus_f, im_width, im_height)
    ...
    error_val_uart=fread(serial_obj,1,'uint8')*(2^24);
    error_val_uart=bitor(error_val_uart,fread(serial_obj,1,'uint8')*(2^16));
    error_val_uart=bitor(error_val_uart,fread(serial_obj,1,'uint8')*(2^8));
    error_val_uart=bitor(error_val_uart,fread(serial_obj,1,'uint8'));
    error_val_uart=typecast(uint32(error_val_uart),'int32');
    error_val_uart=double(error_val_uart)/(2^bus_f);
    %
end
