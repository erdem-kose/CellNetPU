function [] = imshow_fpga(iter, y_uart, cnn_time, x_cpu, x_cpu_time, y_ideal,...
    ...
    subplot(3,3,7)
    imshow(error_map)
    title(sprintf('%s, Error Map\n Error Sum:%d \n Error Square Value Sum: %d',...
            winqueryreg('HKEY_LOCAL_MACHINE','HARDWARE\DESCRIPTION\System\CentralProcessor\0','ProcessorNameString'), error_norm_val, error_squa_val))

    subplot(3,3,8)
    imshow(error_map_uart)
    title(sprintf('Xilinx Spartan 6 Error Map\n Error Sum:%d \n Error Square Value Sum: %d', error_norm_val_uart, error_squa_val_uart))
